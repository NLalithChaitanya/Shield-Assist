...
# Optional but recommended: automatic Groq failover for the copilot
# (chat + grounded drafts keep working when Gemini is down / slow / exhausted)
GROQ_API_KEY=...
...
| Copilot | Gemini → Groq failover | Explain, investigate, recommend, draft |
...
### 2. Gemini free-tier limits → automatic Groq failover

Transient provider errors (503, timeouts) and daily free-tier quota exhaustion can occur on Gemini. The copilot layer handles this in three tiers:

1. **Gemini (primary)** — circuit breaker, rate limiter, and exponential-backoff retries (transient 503s and timeouts are retried; timeouts are classified as recoverable).
2. **Groq (automatic failover)** — on *any* Gemini failure (503, timeout, 429, quota 403, circuit breaker trip) the same grounded request is retried against `GROQ_MODEL`. This covers all four copilot abilities (explain / investigate / recommend / draft) in both the live chat and the background `draft.response` job. Evidence grounding is unchanged: Groq receives the identical structured facts and prompts, and the citation integrity check runs afterward regardless of provider.
3. **Honest fallback** — if both providers fail, Ally returns a message that says it is temporarily unavailable and that the evidence analysis is still available. It never fabricates facts, citations, or document IDs, and this fallback text is never cached as a real answer.

Gemini OCR for document fact extraction (`document.process`) is **not** routed through Groq — it remains Gemini-only.

Scope guardrails: `_call_gemini`/`_call_gemini_stream` in `backend/copilot.py` are the single failover point; public copilot function names and the frontend contract are unchanged. Tests: `tests/test_groq_fallback.py`.

### 2b. Question-aware chat

In the chat UI, Ally answers the merchant's **actual question**, not a fixed ability script: the frontend sends the question and the last few turns (`q=` / `history=` query params on the SSE copilot routes), the backend appends them to the prompt (`_conversation_block` in `backend/copilot.py`), and the response cache is keyed on the question so different questions about the same case produce different answers. Drafting stays safety-gated; grounding rules still forbid inventing facts or citations. Tests: `tests/test_copilot_context.py`.
