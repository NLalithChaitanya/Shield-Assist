...
3. Quality assessment (blur detection via Laplacian variance)
...
### Full multi-condition gate (test set, auto-selected threshold)
...
Metrics above use the probability-calibrated model (isotonic regression). The gate's precision (0.910) is higher than the raw classifier's alone (0.776) — completeness, consistency, and contradiction-freedom filter out exactly the cases where the classifier alone would be wrong, which is the entire point of combining conditions rather than thresholding probability alone.
...
| Total wasted effort (test set) | ₹2,667 | 20 FPs × ₹133 |
| **Wasted effort rate** | **₹2.96 per 100 disputes processed** | ₹2,667 ÷ 901 × 100 |

The rate (₹2.96 per 100 disputes) scales independently of dataset size. In a production environment processing 1,000 disputes/month, this estimates ₹29.60/month in wasted ops effort from false positives — essentially negligible.
...
| **Full multi-condition gate** | **20** | **91.0%** | **₹2,667** | **₹296** |

Reading this table:
- The **full gate avoids ₹5,313 in wasted effort** (40 fewer FPs) compared to using probability alone — this is the direct value of adding completeness, consistency, and contradiction checks on top of the classifier.
- Compared to an "always approve" baseline, the gate eliminates 97% of false positives (654 → 20), saving ₹84,315 per 901 disputes.
...
## The 2-3 minute demo (single dispute, not a feature tour)

1. Dispute arrives — ₹38,500, RZP01, 17h remaining
2. Copilot reads three uploaded documents
3. Case strength: 64/100 — "missing delivery proof"
4. Merchant uploads delivery receipt
5. System detects: delivery recipient matches customer, order amount matches, delivery date precedes dispute — consistency check passes
6. Score updates: 64 → 91
7. Copilot drafts a grounded response
8. Click a sentence → source document opens (citation)
9. System says: "Safe to prepare for submission" — gate conditions all met
10. Audit trail records the full sequence

Then, as a second beat: show a contradictory case where the gate correctly blocks auto-submission despite a high raw probability — this is the moment that proves "bounded," not just "confident."
